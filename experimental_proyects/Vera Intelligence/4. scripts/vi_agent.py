...
# ≤12 caracteres que SÍ deba tratarse como interno, agregarlo acá en vez de bajar el umbral
# general.
#
# CORREGIDO (2026-09-23, hallazgo real auditando los Data Maps actuales -ya no son 19, la
# afirmación "ninguno encontrado" quedó desactualizada con la incorporación de Steren):
# "secompro" (8 caracteres, campo de categorical_product_insights) y "motivocompra" (12
# caracteres exactos, en el límite del umbral ">12") son nombres de campo comprimidos que nadie
# diría así en una charla real -"secompro" no es una palabra ("¿se compró?" sí lo es, pero no
# concatenada), y "motivocompra" es "motivo de compra" sin espacios ni preposición-. Ninguno de
# los dos pasaba `_is_internal_identifier` (ambos ≤12), así que si el modelo alguna vez los
# escribe tal cual en una respuesta sobre Steren, el filtro de caja negra no lo habría detectado.
_FORCE_BLOCK_SHORT_IDENTIFIERS: frozenset[str] = frozenset({"secompro", "motivocompra"})
...
- NO CALCULES UNA TASA/PORCENTAJE QUE LA PREGUNTA NO PIDIÓ, sólo para "dar contexto" (hallazgo real,
  2026-09-23: ante una pregunta puramente cualitativa -"qué quejas u objeciones se repiten"-, el
  modelo agregó por su cuenta una tasa de manejo de objeciones no pedida; esa cifra falló la
  verificación 3 veces seguidas y, al agotarse los reintentos, se descartó la respuesta COMPLETA
  -incluyendo el contenido cualitativo real y ya verificado que sí respondía la pregunta-. Un número
  de más, mal verificado, puede tirar abajo una respuesta que ya estaba bien resuelta sin él). Si la
  pregunta es cualitativa y no pide una tasa o porcentaje, no la agregues por iniciativa propia -un
  conteo simple ya resuelto en una consulta (sin fórmula derivada) es más seguro que una tasa nueva
  que exige su propio bloque de evidencia. Cuantificá sólo lo que la pregunta pide o lo mínimo
  indispensable para dimensionar la respuesta, nunca como adorno.
...
- Para tasas, promedios y puntuaciones, devolvé también la cantidad efectivamente evaluada (alias n_evaluados o base_evaluada), excluyendo N/A y NULL según el mapa. Si cambia por criterio/grupo, mantené la base correspondiente para cada uno. Mostrá esa base cuando sea pertinente; si no está disponible, aclaralo. Un ranking o porcentaje sin base conocida no demuestra solidez, representatividad ni causalidad. Una observación no permite generalizar. No inventes un mínimo de muestra ni niveles de confianza. FORMATO (corregido 2026-09-23, visto en vivo: un párrafo entero de "Base evaluada de los indicadores citados: 186 conversaciones. Base evaluada de los indicadores citados: 123 conversaciones..." repitiendo la misma frase 8 veces con distinto número, sin decir a qué criterio correspondía cada uno -ilegible y sin indicar de qué era base de qué): la base va SIEMPRE integrada en la misma línea/viñeta que su cifra, entre paréntesis o como parte de la misma oración (ej. "cierre de venta: 45% (85 de 186 conversaciones evaluadas)"), nunca como una oración aparte, y mucho menos como una lista de oraciones repetidas con la misma frase de relleno para cada criterio -si hay varios criterios con base distinta, cada uno lleva SU base en SU propia línea, no una detrás de otra en un párrafo separado.
...
def _build_extra_tools_section() -> str:
    ...
    if CLIENT_CONFIG.vector_search:
        items.append(
            ...
            "quedaba enterrada a mitad de oración, invisible en una lectura rápida). Regla: la "
            "etiqueta fija en negrita **En conversaciones reales:** marca UNA VEZ el inicio del "
            "bloque que viene de acá, no cada viñeta -repetirla en cada línea de una lista ya "
            "agrupada bajo un título propio (ej. 'Patrones cualitativos observados') quedó repetitivo "
            "y mecánico (corregido 2026-09-23 tras verlo en vivo). Dos casos:\n"
            "      · Si agrupás 2 o más notas/patrones en su propia sección o lista con título "
            "-'Hallazgos cualitativos', 'Patrones observados'- el título YA cumple el rol de marcar "
            "el origen: no agregues encima la etiqueta **En conversaciones reales:** pegada al "
            "título ni al principio del párrafo que sigue (nunca 'Patrones cualitativos observados: "
            "En conversaciones reales: ...' -doble marca fusionada en una sola oración, corregido "
            "2026-09-23 tras verlo en vivo, se lee como un título roto, no como dos señales útiles). "
            "Usá SÓLO UNA de las dos formas, nunca ambas concatenadas: (a) un título de sección que ya "
            "sea inequívoco por sí solo, sin la etiqueta después, o (b) sin título de sección, la "
            "etiqueta como intro de un párrafo o lista normal. Si dudás, preferí (a) -es más limpio "
            "visualmente- y dejá la etiqueta sólo para el caso de la oración suelta de abajo.\n"
            "      · Si mezclás una sola observación cualitativa suelta en medio de un párrafo que "
            "también tiene números de SQL (el caso real que motivó esto), ahí SÍ marcá esa oración "
            "puntual con la etiqueta al principio, para no perder la distinción con el número de al "
            "lado. Ej.: '**En conversaciones reales:** ante la falta de talla, se deriva a pedido "
            "especial o confección a medida, pero parte de los clientes desiste por la urgencia de su "
            "ocasión de uso.'\n"
            "      Esto aplica en TODAS las secciones que usan esta tool (POR QUÉ, BÚSQUEDA DE "
            "PATRONES, EQUIPO EN UN PERÍODO) excepto el bloque de coaching por situación, que ya usa "
            "su propia etiqueta en negrita ('**Cuando [situación]**') como marca equivalente.\n"
