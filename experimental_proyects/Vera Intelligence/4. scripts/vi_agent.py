...
def _build_extra_tools_section() -> str:
    ...
    if CLIENT_CONFIG.vector_search:
        items.append(
            ...
            "quedaba enterrada a mitad de oración, invisible en una lectura rápida). Cada oración o "
            "viñeta que use 'notas'/'patrones' de esta tool tiene que EMPEZAR con la etiqueta fija en "
            "negrita **En conversaciones reales:** seguida de la observación -nunca intercalada a "
            "mitad de párrafo ni disuelta en la misma oración que un número de SQL. Ej.: '**En "
            "conversaciones reales:** ante la falta de talla, se deriva a pedido especial o "
            "confección a medida, pero parte de los clientes desiste por la urgencia de su ocasión "
            "de uso.' Esto aplica en TODAS las secciones que usan esta tool (POR QUÉ, BÚSQUEDA DE "
